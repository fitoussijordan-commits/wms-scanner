...
export async function checkRecipient(session: OdooSession, partnerId: number): Promise<RecipientCheck | null> {
  ...
  const rows = await searchRead(session, M("MODEL_PARTNER"), [["id", "=", partnerId]], ["id", "name"], 1);
  const name: string = rows[0]?.name || "";
  if (!name) return null;
  const refuses = Array.from(new Set((name.match(/['’`]/g) || [])));
  const tropLong = name.length > RECIPIENT_MAX;
  return {
    partnerId, name, tropLong,
    caracteresRefuses: refuses,
    ok: !tropLong && refuses.length === 0,
    suggestion: sanitizeRecipient(name),
  };
}

/** Applique le nom validé par l'opérateur sur la fiche client. */
export async function renamePartner(session: OdooSession, partnerId: number, name: string): Promise<void> {
  await write(session, M("MODEL_PARTNER"), [partnerId], { name: name.trim() });
}
