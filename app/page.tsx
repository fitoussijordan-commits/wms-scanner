...
async function generatePackingListPDF(chain: {
  ...
  senderName: string; unit: string; orderRef: string;
  palettes: Array<{ id: string; lines: Array<{ ref: string; lot: string; qty: string }> }>;
}): Promise<void> {
  const { jsPDF } = await import("jspdf");
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  ...
  // ── Colors ──
  const DARK_BLUE  = [31, 73, 125]   as [number,number,number];
  const MID_BLUE   = [68, 114, 196]  as [number,number,number];
  const LIGHT_BLUE = [189, 215, 238] as [number,number,number];
  const YELLOW     = [255, 242, 102] as [number,number,number];
  const TOTAL_BG   = [213, 226, 242] as [number,number,number];
  ...
  doc.text("PACKING LIST  /  LISTE DE COLISAGE", W / 2, 11.5, { align: "center" });
  y = 26;
  ...
  doc.roundedRect(margin, y, half, 30, 2, 2, "FD");
  // Right box — Destinataire
  doc.roundedRect(margin + half + 6, y, half, 30, 2, 2, "FD");

  // Labels
  doc.setFontSize(7.5);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...MID_BLUE);
  doc.text("EXPÉDITEUR", margin + 4, y + 5);
  doc.text("DESTINATAIRE", margin + half + 10, y + 5);

  // Separator line under labels
  doc.setDrawColor(...LIGHT_BLUE);
  doc.line(margin + 4, y + 6.5, margin + half - 4, y + 6.5);
  doc.line(margin + half + 10, y + 6.5, margin + contentW - 4, y + 6.5);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(...BLACK);
  const senderLines = (chain.senderName || "WALA France").split("\n");
  senderLines.forEach((l, i) => doc.text(l, margin + 4, y + 11 + i * 4.5));

  const recipLines = [chain.recipientName, chain.recipientAddress].filter(Boolean) as string[];
  doc.setFont("helvetica", "bold");
  doc.text(recipLines[0] || "", margin + half + 10, y + 11);
  doc.setFont("helvetica", "normal");
  (recipLines.slice(1) as string[]).forEach((l, i) => doc.text(l, margin + half + 10, y + 15.5 + i * 4.5));
  y += 36;
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...BLACK);
  doc.text(new Date().toLocaleDateString("fr-FR"), margin + 13, y + 5);
  if (chain.orderRef) {
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...DARK_BLUE);
    doc.text("N° CDE :", margin + 60, y + 5);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...BLACK);
    doc.text(chain.orderRef, margin + 79, y + 5);
  }
  y += 13;

  // ── Table ──
  const cols = [
    { label: "N°",          w: 14,  align: "center" as const },
    { label: "Code Article",w: 32,  align: "center" as const },
    { label: "Description", w: 72,  align: "left"   as const },
    { label: "Lot",         w: 36,  align: "center" as const },
    { label: "Quantité",    w: 28,  align: "center" as const },
  ];
  // Compute x positions
  let cx = margin;
  const colXs = cols.map(c => { const x = cx; cx += c.w; return x; });
  ...
  doc.setTextColor(...WHITE);
  cols.forEach((c, i) => {
    const tx = c.align === "center" ? colXs[i] + c.w / 2 : colXs[i] + 2;
    doc.text(c.label, tx, y + 5.5, { align: c.align === "left" ? "left" : "center" });
  });
  y += headerH;

  // Rows
  const allRows: Array<{ pNum: number; ref: string; lot: string; qty: number }> = [];
  chain.palettes.forEach((pal, pi) => {
    pal.lines.filter(l => l.ref.trim()).forEach(l => {
      allRows.push({ pNum: pi + 1, ref: l.ref.trim(), lot: l.lot?.trim() || "", qty: Number(l.qty) || 0 });
    });
  });

  allRows.forEach((row, ri) => {
    const isOdd = row.pNum % 2 === 1;
    doc.setFillColor(...(isOdd ? YELLOW : LIGHT_BLUE));
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.5);
    doc.setTextColor(isOdd ? 120 : 40, isOdd ? 80 : 80, isOdd ? 0 : 120);
    doc.text("P" + row.pNum, colXs[0] + cols[0].w / 2, y + 4.8, { align: "center" });
  ...
  stats.forEach(([label, val]) => {
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...DARK_BLUE);
    doc.text(label, margin + 50, y, { align: "right" });
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...BLACK);
    doc.text(val, margin + 53, y);
    y += 5.5;
  });
  ...
  doc.text("Document généré automatiquement — WMS Scanner", W / 2, 284, { align: "center" });

  const blob = doc.output("blob");
  window.open(URL.createObjectURL(blob), "_blank");
}

// ── Packing list PDF Palette (A4, une seule palette multi-produits) ──────────
async function generatePalettePackingListPDF(pal: {
  senderName: string; senderAddress: string;
  recipientName: string; recipientAddress: string;
  orderRef: string; deliveryRef: string;
  refs: Array<{ productName: string; ref: string; lotNumber: string; expiryDate: string; quantity: number | string; unit: string; unitsPerCarton: string; unitWeight: string; weight: string }>;
}): Promise<void> {
  const { jsPDF } = await import("jspdf");
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const W = 210; const margin = 14;
  ...
  const WHITE      = [255, 255, 255] as [number,number,number];
  const BLACK      = [0, 0, 0]       as [number,number,number];
  const ROW_ALT    = [245, 248, 255] as [number,number,number];
  ...
  doc.roundedRect(margin, y, half, 30, 2, 2, "FD");
  doc.roundedRect(margin + half + 6, y, half, 30, 2, 2, "FD");
  doc.setFontSize(7.5); doc.setFont("helvetica", "bold"); doc.setTextColor(...MID_BLUE);
  doc.text("EXPÉDITEUR", margin + 4, y + 5);
  doc.text("DESTINATAIRE", margin + half + 10, y + 5);
  doc.setDrawColor(...LIGHT_BLUE);
  doc.line(margin + 4, y + 6.5, margin + half - 4, y + 6.5);
  ...
  const sLines = (pal.senderName || "WALA France").split("\n");
  sLines.forEach((l, i) => doc.text(l, margin + 4, y + 11 + i * 4.5));
  if (pal.senderAddress) doc.text(pal.senderAddress, margin + 4, y + 11 + sLines.length * 4.5);
  doc.setFont("helvetica", "bold");
  doc.text(pal.recipientName || "", margin + half + 10, y + 11);
  doc.setFont("helvetica", "normal");
  if (pal.recipientAddress) doc.text(pal.recipientAddress, margin + half + 10, y + 15.5);
  y += 36;

  // Meta row
  doc.setFont("helvetica", "bold"); doc.setFontSize(8.5); doc.setTextColor(...DARK_BLUE);
  doc.text("Date :", margin, y + 5);
  doc.setFont("helvetica", "normal"); doc.setTextColor(...BLACK);
  doc.text(new Date().toLocaleDateString("fr-FR"), margin + 13, y + 5);
  let metaX = margin + 50;
  if (pal.orderRef) {
    doc.setFont("helvetica", "bold"); doc.setTextColor(...DARK_BLUE);
    doc.text("N° CDE :", metaX, y + 5); metaX += 19;
    doc.setFont("helvetica", "normal"); doc.setTextColor(...BLACK);
    doc.text(pal.orderRef, metaX, y + 5); metaX += doc.getTextWidth(pal.orderRef) + 8;
  }
  if (pal.deliveryRef) {
    doc.setFont("helvetica", "bold"); doc.setTextColor(...DARK_BLUE);
    doc.text("N° BL :", metaX, y + 5); metaX += 15;
    doc.setFont("helvetica", "normal"); doc.setTextColor(...BLACK);
    doc.text(pal.deliveryRef, metaX, y + 5);
  }
  y += 13;

  // Table columns
  const cols = [
    { label: "N°",         w: 10,  align: "center" as const },
    { label: "Produit",    w: 48,  align: "left"   as const },
    { label: "Référence",  w: 24,  align: "center" as const },
    { label: "Lot",        w: 24,  align: "center" as const },
    { label: "DLUO",       w: 22,  align: "center" as const },
    { label: "Qté",        w: 14,  align: "center" as const },
    { label: "Unité",      w: 16,  align: "center" as const },
    { label: "Poids",      w: 24,  align: "center" as const },
  ];
  let cx = margin;
  const colXs = cols.map(c => { const x = cx; cx += c.w; return x; });
  const rowH = 7; const headerH = 8;
  ...
  // Data rows
  let totalQty = 0; let totalWeight = 0;
  const validRefs = pal.refs.filter(r => r.productName || r.ref);
  validRefs.forEach((ref, ri) => {
    const qty = Number(ref.quantity) || 0;
    totalQty += qty;
    const wNum = parseFloat(ref.weight);
    if (!isNaN(wNum)) totalWeight += wNum;

    doc.setFillColor(...(ri % 2 === 0 ? WHITE : ROW_ALT));
    doc.rect(margin, y, contentW, rowH, "F");
    doc.setDrawColor(...GRAY_LINE); doc.setLineWidth(0.2);
    doc.line(margin, y + rowH, margin + contentW, y + rowH);

    doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); doc.setTextColor(...BLACK);
    doc.text(String(ri + 1), colXs[0] + cols[0].w / 2, y + 4.8, { align: "center" });
    const prodFit = doc.splitTextToSize(ref.productName || "", cols[1].w - 3);
    doc.text(prodFit[0] || "", colXs[1] + 2, y + 4.8);
    doc.text(ref.ref || "", colXs[2] + cols[2].w / 2, y + 4.8, { align: "center" });
    doc.text(ref.lotNumber || "", colXs[3] + cols[3].w / 2, y + 4.8, { align: "center" });
    doc.text(ref.expiryDate ? ref.expiryDate.substring(0, 10) : "", colXs[4] + cols[4].w / 2, y + 4.8, { align: "center" });
    doc.text(qty > 0 ? String(qty) : "", colXs[5] + cols[5].w / 2, y + 4.8, { align: "center" });
    doc.text(ref.unit || "", colXs[6] + cols[6].w / 2, y + 4.8, { align: "center" });
    doc.text(ref.weight ? ref.weight.replace(" kg", "") + " kg" : "", colXs[7] + cols[7].w / 2, y + 4.8, { align: "center" });
    y += rowH;
    if (y > 260) { doc.addPage(); y = margin; }
  });
  ...
  doc.setFillColor(...TOTAL_BG);
  doc.rect(margin, y, contentW, rowH + 1, "F");
  doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(...DARK_BLUE);
  doc.text("TOTAL", colXs[4] + cols[4].w / 2, y + 5.5, { align: "center" });
  doc.text(String(totalQty), colXs[5] + cols[5].w / 2, y + 5.5, { align: "center" });
  if (totalWeight > 0) doc.text(`${totalWeight.toFixed(2)} kg`, colXs[7] + cols[7].w / 2, y + 5.5, { align: "center" });
  y += rowH + 9;

  // Footer
  doc.setDrawColor(...MID_BLUE); doc.setLineWidth(0.5);
  doc.line(margin, 280, W - margin, 280);
...
function getPendingLinesAtLoc(lines: any[], locId: number): any[] {
  const totals = new Map<number, { reserved: number; done: number }>();
  for (const m of lines) {
    if (m.location_id?.[0] !== locId) continue;
    const pid: number = m.product_id?.[0]; if (!pid) continue;
    const c = totals.get(pid) || { reserved: 0, done: 0 };
    c.reserved += (m.reserved_uom_qty || 0);
    c.done     += (m.qty_done || 0);
    totals.set(pid, c);
  }
...
function getProductRemainingAtLoc(lines: any[], locId: number, productId: number): number {
  let reserved = 0, done = 0;
  for (const m of lines) {
    if (m.location_id?.[0] !== locId || m.product_id?.[0] !== productId) continue;
    reserved += (m.reserved_uom_qty || 0);
    done     += (m.qty_done || 0);
  }
  return Math.max(0, reserved - done);
}
...
export default function Page() {
  ...
  // Outils masqués globalement (gérés depuis Administration) → cachés du menu pour tous.
  const [hiddenTools, setHiddenTools] = useState<string[]>([]);
  useEffect(() => {
    if (!session) return;
    sbase.loadHiddenTools().then(setHiddenTools).catch(() => setHiddenTools([]));
  }, [session]);
  ...
  const loadBadges = useCallback(async () => {
    ...
    const [waiting, eshop, returns, negStock, packing] = await Promise.allSettled([
      ...
      // 2. E-shop — commandes SendCloud filtrées (status "0" + intégration 527093 + MR/Colissimo)
      //    EXCLUT les commandes déjà préparées (cohérent avec la liste e-shop).
      (async () => {
    ...
    try {
      ...
      if (!currentStep) {
        if (r.type === "location") {
          ...
          // Utilise getPendingLinesAtLoc : tient compte des déviations (total produit)
  ...
  const canSee = (key: string): boolean => {
    // Masquage global (géré depuis Administration) : s'applique à tout le monde,
    // sauf "admin" — sinon on ne pourrait plus rouvrir l'écran pour le réactiver.
    if (key !== "admin" && hiddenTools.includes(key)) return false;
    ...
    <Shell toast={toast} flash={scanFlash} desktop={isDesktopUI}>
      ...
      <main style={isDesktopUI
       ...
       <div style={isDesktopUI ? { maxWidth: screen === "home" ? 1240 : 1500, margin: "0 auto" } : undefined}>
        ...
        {screen === "transfer" && <>
          ...
          {transferMode === "quick" && <>
            ...
            {curProduct && src && !dst && (
              <Section>
                ...
                {/* Suggestions : emplacements où ce produit est déjà en stock (groupés, >=3 niveaux) */}
                {(() => {
                  const suggestMap: Record<number, { loc: any; qty: number }> = {};
                  for (const q of allStock) {
                    if (q.location_id[0] === src.id || q.quantity <= 0) continue;
                    const loc = locations.find((l: any) => l.id === q.location_id[0]);
                    if (!loc || loc.usage !== "internal") continue;
                    if ((loc.complete_name || "").split("/").length < 3) continue; // exclure WH/Sortie etc.
                    if (!suggestMap[q.location_id[0]]) suggestMap[q.location_id[0]] = { loc, qty: 0 };
                    suggestMap[q.location_id[0]].qty += q.quantity;
                  }
                  const suggestions = Object.values(suggestMap);
                  ...
                  return (
                    <div style={{ marginBottom: 10 }}>
                      ...
                      {suggestions.map((s, i) => (
                        <button key={i} onClick={() => { setDst(s.loc); vibrateSuccess(); }}
                          ...
                          <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{s.loc.complete_name || s.loc.name}</span>
                          <span style={{ fontSize: 12, fontWeight: 700, color: C.green }}>{s.qty} en stock</span>
                        </button>
                      ))}
          ...
          {transferMode === "classic" && lines.length > 0 && (
            <Section>
              ...
              {lines.map((l, i) => (
                <EditableLine key={i} line={l}
                  onDelete={() => setLines(p => p.filter((_, j) => j !== i))}
                  onUpdate={(u: any) => setLines(p => p.map((x, j) => j === i ? { ...x, ...u } : x))}
                />
              ))}
            </Section>
          )}
        ...
        {screen === "eshopSorties" && session && (
          <EshopSortiesScreen session={session} onBack={goHome} onToast={showToast} />
        )}
        ...
        {screen === "fefo" && session && odoo.isAdmin(session) && (
          <FefoAnalysisScreen session={session} onBack={goHome} onToast={showToast} />
        )}
...
// EDITABLE LINE
// ============================================
function EditableLine({ line, onDelete, onUpdate }: { line: any; onDelete: () => void; onUpdate: (u: any) => void }) {
  ...
  if (editing) return (
    <div style={{ padding: "12px 0", borderBottom: `1px solid ${C.border}` }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 4, flex: 1, minWidth: 80 }}>
          <span style={{ fontSize: 11, color: C.textMuted }}>Qté</span>
          <input style={inputStyle} type="number" value={qty} onChange={e => setQty(e.target.value)} autoFocus />
        </div>
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 4, flex: 2, minWidth: 120 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={save} style={{ padding: "10px 14px", background: C.green, color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, cursor: "pointer" }}>✓</button>
          <button onClick={() => setEditing(false)} style={{ padding: "10px 14px", background: C.border, color: C.text, border: "none", borderRadius: 8, cursor: "pointer" }}>✕</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: `1px solid ${C.border}` }}>
      ...
      <div style={{ flex: 1 }} onClick={() => setEditing(true)} role="button">
...
function LocationDropdown({ locations, onSelect, excludeId }: { locations: any[]; onSelect: (loc: any) => void; excludeId?: number }) {
  ...
  const trySelect = (val: string) => {
    ...
    if (exact) { onSelect(exact); setSearch(""); return true; }
    return false;
  };
  ...
  return (
    <div>
      <input
        ref={inputRef}
        style={{ ...inputStyle, borderColor: C.blue }}
        value={search}
        onChange={e => { const v = e.target.value; setSearch(v); trySelect(v); }}
        onKeyDown={e => {
          e.stopPropagation();
          if (e.key === "Enter") {
            if (trySelect(search)) return;
            if (filtered.length === 1) { onSelect(filtered[0]); setSearch(""); }
          }
        }}
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (tab === "blank") {
        ...
        result = await pn.printPdfLabel(printerId, pdfB64, "Étiquette vierge", qty);
      } else {
        ...
        const palData: pn.PaletteLabelData = {
          ...
          weight: (() => {
          const total = (pal.refs as any[]).reduce((sum, r) => {
            const w = parseFloat(r.weight);
            return isNaN(w) ? sum : sum + w;
          }, 0);
          return total > 0 ? `${total.toFixed(2)} kg` : undefined;
        })(),
      ...
      <div style={{ background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, padding: 14, marginBottom: 14, boxShadow: C.shadow }}>
        ...
        {tab === "blank" && (
          <div>
            ...
            <LabelEditor
              ...
              onPrint={async (pdfB64) => {
                const cfg = pn.getLabelTypeConfig("blank");
                const pid = cfg.printerId || pn.getSavedPrinterId();
                if (!pid) { onToast("⚠️ Sélectionne une imprimante"); return; }
                const r = await pn.printPdfLabel(pid, pdfB64, "Étiquette vierge", qty);
                if (r.success) onToast("✅ Impression envoyée");
                else onToast("❌ " + (r.error || "Erreur"));
              }}
              printing={loading}
            />
          </div>
        ...
        {tab === "palette" && (
          <div>
            ...
            <button onClick={() => generatePalettePackingListPDF(pal)}
              style={{ width: "100%", padding: "11px", borderRadius: 10, border: `1.5px solid ${C.green}`,
                background: "#f0fdf4", color: C.green, fontSize: 14, fontWeight: 700,
                fontFamily: "inherit", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              📋 Générer Packing List A4
            </button>
          </div>
        )}
      </div>
...
function EshopScreen({ session, onBack, onToast }: { session: any; onBack: () => void; onToast: (m: string) => void }) {
  ...
  const handleWaveScan = useCallback((code: string) => {
    const waveItems = buildWaveItems();
    const trimmed = code.trim();
    setWaveScanError("");
    const currentItem = waveItems.find(it => (waveScannedSkus[it.sku] || 0) < it.totalQty);
    if (!currentItem) return;
    if (!currentItem._isChariot) {
      if (!waveLocConfirmed) {
        const locShort = currentItem.location.toLowerCase();
        const locFull = currentItem.locationFull.toLowerCase();
        const s = trimmed.toLowerCase();
        const segments = s.split(/[^a-z0-9]+/).filter((p: string) => p.length >= 2);
        const matched = s === locShort || s === locFull || locFull.includes(s) || locShort.startsWith(s) || segments.some((seg: string) => locShort.startsWith(seg) || locShort === seg);
        if (matched) { setWaveLocConfirmed(true); return; }
        else { setWaveScanError(`❌ Emplacement "${trimmed}" — attendu: ${currentItem.location || "?"}`); vibrateError(); return; }
      }
    }
    const matched = waveItems.find(it => {
      const match = getMatch(it.sku);
      return it.sku === trimmed || match?.default_code === trimmed || match?.barcode === trimmed;
    });
    if (!matched) { setWaveScanError(`❌ "${trimmed}" — article non trouvé dans la liste`); vibrateError(); return; }
  ...
  const renderPrepTab = () => {
    ...
    // Exclure les commandes en cours de wave picking (liste figée)
    const pending = parcels.filter(p => !preparedIds.has(p.order_number) && !(waveActive && waveOrders.has(p.order_number)));
    ...
    return (
      <>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>Progression</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: allDone ? C.green : C.blue }}>{scannedCount}/{items.length} articles</span>
          </div>
          <div style={{ height: 8, borderRadius: 4, background: C.bg, overflow: "hidden", border: `1px solid ${C.border}` }}>
        ...
        {!allDone && remainingItems.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: C.textMuted, marginBottom: 8, textTransform: "uppercase" as const, letterSpacing: 0.5 }}>Restant ({remainingItems.length})</div>
...
function ProductImportScreen({ session, onBack, onToast, desktop }: { session: any; onBack: () => void; onToast: (m: string) => void; desktop?: boolean }) {
  const [tab, setTab] = useState<"import" | "create" | "logistique" | "seuils" | "nonvendable" | "abloquer" | "mea">("import");
  ...
  return (
    <div style={desktop ? { padding: 0 } : { padding: "20px 16px", maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={desktop
        ...
        {([["import", "Import Excel"], ["mea", "Création MEA"], ["create", "Créer article"], ["logistique", "EAN / Poids / Dim."], ["seuils", "Seuils d'alerte"], ["nonvendable", "Stock non vendable"], ["abloquer", "Stock à bloquer"]] as const).map(([t, label]) => (
      ...
      {tab === "seuils" ? (
        ...
        <div style={desktop ? { maxWidth: 860, margin: "0 0 0 -16px" } : undefined}>
...
function ReprintLabelScreen({ session, onBack, onToast }: { session: any; onBack: () => void; onToast: (m: string) => void }) {
  ...
  const [packages, setPackages] = useState<Record<number, any[]>>({});
  ...
  const loadDetail = async (pickingId: number, force = false) => {
    if (!force && attachments[pickingId]) return;
    setLoadingDetail(prev => ({ ...prev, [pickingId]: true }));
    try {
      const [att, pkgs] = await Promise.all([
        odoo.getPickingAttachments(session, pickingId),
        odoo.getPickingPackages(session, pickingId),
      ]);
      // Enrich packages with weight from stock.quant.package
      let pkgsEnriched = pkgs;
      if (pkgs.length > 0) {
        try {
          const pkgDetails = await odoo.searchRead(session, await odoo.packageModel(session),
      ...
      setPackages(prev => ({ ...prev, [pickingId]: pkgsEnriched }));
    } catch (e: any) { onToast("❌ " + e.message); }
    setLoadingDetail(prev => ({ ...prev, [pickingId]: false }));
  };
  ...
  const printAttachment = async (att: any) => {
  ...
  const resendToShipper = async (picking: any) => {
    ...
    try {
      ...
      onToast("✅ Demande envoyée — rechargement dans 3s…");
      setTimeout(() => {
        setAttachments(prev => { const n = { ...prev }; delete n[picking.id]; return n; });
        setPackages(prev => { const n = { ...prev }; delete n[picking.id]; return n; });
        loadDetail(picking.id, true);
      }, 3000);
    } catch (e: any) { onToast("❌ " + e.message); }
  ...
  const addPackage = async (picking: any) => {
    ...
    setAddingPkg(prev => ({ ...prev, [picking.id]: true }));
    try {
      onToast("📦 Création du colis et envoi TNT…");
      const { packageId, attachments: newAtts } = await odoo.addPackageAndSendToShipper(session, picking.id, w);
      onToast(`✅ Colis créé (${w} kg) — ${newAtts.length > 0 ? newAtts.length + " nouvelle(s) étiquette(s)" : "Rechargement…"}`);
      // Auto-print if new attachments found
      if (newAtts.length > 0) {
        const cfg = pn.getLabelTypeConfig("blank");
        const pid = cfg.printerId || pn.getSavedPrinterId();
        if (pid) {
          for (const att of newAtts) {
            try { await pn.printPdfLabel(pid, att.datas, att.name); } catch {}
          }
        }
      }
      setNewWeight(prev => ({ ...prev, [picking.id]: "" }));
      // Reload detail
      setAttachments(prev => { const n = { ...prev }; delete n[picking.id]; return n; });
      setPackages(prev => { const n = { ...prev }; delete n[picking.id]; return n; });
      await loadDetail(picking.id, true);
    } catch (e: any) { onToast("❌ " + e.message); }
...
function InventoryScreen({ session, onBack, onToast, initialProduct, desktop }: { session: any; onBack: () => void; onToast: (m: string) => void; initialProduct?: { id: number; productName: string } | null; desktop?: boolean }) {
  ...
  const [newStockExpiry, setNewStockExpiry] = useState(""); // YYYY-MM-DD (optionnel)
  const [newStockSearching, setNewStockSearching] = useState(false);
  // Formulaire "ajouter une ligne" (produit avec stock existant — nouveau lot/emplacement)
  ...
  const lookupNewStockLoc = () => {
    if (!newStockLoc.trim()) return;
    ...
    odoo.searchRead(session, "stock.location", [["complete_name", "ilike", newStockLoc], ["usage", "=", "internal"]], ["id", "complete_name", "name"], 10)
      .then(res => {
        if (res.length >= 1) {
          setNewStockLocId(res[0].id);
          setNewStockLoc(res[0].complete_name || res[0].name);
          if (res.length > 1) onToast(`${res.length} emplacements trouvés, 1er sélectionné`);
        } else onToast("Emplacement introuvable");
      })
      .catch(e => onToast("Erreur: " + e.message))
      .finally(() => setNewStockSearching(false));
  };

  // Crée une nouvelle ligne de stock (nouveau lot et/ou emplacement) sur le produit sélectionné
  const addNewStockLine = async () => {
    ...
    try {
      ...
      if (newStockLot.trim()) {
        const { id } = await odoo.getOrCreateLot(session, selectedProduct.id, newStockLot.trim(), newStockExpiry || "");
        lotId = id;
      }
      await odoo.createInventoryAdjustment(session, selectedProduct.id, newStockLocId, qty, lotId, adjustReason || undefined);
      onToast(`✅ Ligne créée : ${qty} × ${newStockLot.trim() || "(sans lot)"} → ${shortLocName(newStockLoc)}`);
      ...
      setAdjustments(init);
      setAddLineOpen(false);
      setNewStockLoc(""); setNewStockLocId(null); setNewStockQty("1"); setNewStockLot(""); setNewStockExpiry("");
    } catch (e: any) { onToast("Erreur: " + e.message); }
    setSaving(false);
  };
                        ...
                        {item.quants.map(q => {
                          ...
                          return (
                            <div key={q.id} style={{ display: "flex", alignItems: "center", gap: 10, background: C.white, border: `1px solid ${changed ? C.blueBorder : C.border}`, borderRadius: 8, padding: "8px 12px" }}>
                              ...
                              {changed && <span style={{ fontSize: 12, fontWeight: 700, color: newQty >= 0 ? C.blue : C.red, fontFamily: "monospace", minWidth: 70 }}>→ <b>{Math.max(0, newQty)}</b></span>}
                              {changed && <button onClick={() => setBulkDelta(idx, q.id, 0)} style={{ background: "none", border: "none", cursor: "pointer", color: C.textMuted, fontSize: 13, padding: "0 2px", fontFamily: "inherit" }}>↺</button>}
                            </div>
                          );
                        })}
                      </div>
              ...
              <div style={{ marginTop: 16, display: "flex", gap: 10, justifyContent: "flex-end", borderTop: `1px solid ${C.border}`, paddingTop: 16 }}>
                <div style={{ flex: 1, fontSize: 12, color: C.textMuted, alignSelf: "center" }}>
                  {bulkHasChanges
                    ...
                    : "Utilise + ou − sur chaque ligne"}
                </div>
                <button onClick={applyBulkAdjustments} disabled={bulkSaving || !bulkHasChanges}
                  style={{ padding: "10px 24px", borderRadius: 9, border: "none", background: bulkHasChanges ? C.green : C.border, color: "#fff", fontSize: 14, fontWeight: 700, cursor: bulkHasChanges ? "pointer" : "default", fontFamily: "inherit" }}>
      ...
      {(selectedProduct || selectedLocation) && (
        <>
          <Section>
              ...
              <div>
                {selectedLocation ? (
                  ...
                  <>
                    ...
                    {selectedProduct.lotName && <div style={{ fontSize: 12, color: C.blue, fontWeight: 600 }}>Lot: {selectedProduct.lotName}</div>}
                  </>
                )}
              </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" as const }}>
                  ...
                  <div style={{ flex: "1 1 130px" }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, textTransform: "uppercase" as const, letterSpacing: "0.05em", marginBottom: 4 }}>DLUO <span style={{ fontWeight: 400 }}>(si nouveau lot)</span></div>
                    <input type="date" value={newStockExpiry} onChange={e => setNewStockExpiry(e.target.value)}
                      onKeyDown={e => e.stopPropagation()}
                      style={{ width: "100%", padding: "9px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 14, fontFamily: "inherit", background: C.white, color: C.text, boxSizing: "border-box" as const }} />
                  </div>
                </div>
                {/* Quantité */}
                <div style={{ marginBottom: 20 }}>
                ...
                <button disabled={saving || !newStockLocId || !newStockQty || parseFloat(newStockQty) <= 0}
                  onClick={async () => {
                    ...
                    try {
                      let lotId: number | undefined;
                      if (newStockLot.trim()) {
                        // Cherche ou crée le lot (avec DLUO si fournie)
                        const { id } = await odoo.getOrCreateLot(session, selectedProduct.id, newStockLot.trim(), newStockExpiry || "");
                        lotId = id;
                      }
            ...
            {/* ── Ajouter une ligne (nouveau lot / nouvel emplacement) ── */}
            {!loadingQuants && selectedProduct && quants.length > 0 && (
              !addLineOpen ? (
                <button
                  onClick={() => { setNewStockLoc(""); setNewStockLocId(null); setNewStockQty("1"); setNewStockLot(""); setNewStockExpiry(""); setAddLineOpen(true); }}
                  style={{ marginTop: 12, width: "100%", padding: "11px 0", background: "none", border: `1.5px dashed ${desktop ? "#c7d2e3" : C.border}`, borderRadius: 10, cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700, color: C.blue, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                  Ajouter une ligne — nouveau lot ou emplacement
                </button>
              ) : (
                <div style={{ marginTop: 12, padding: 16, border: `1.5px solid ${C.blue}`, borderRadius: 12, background: "#f8faff" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <div style={{ fontSize: 13.5, fontWeight: 800, color: C.text }}>Nouvelle ligne de stock</div>
                    <button onClick={() => setAddLineOpen(false)} style={{ fontSize: 11, color: C.textMuted, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit" }}>✕ Annuler</button>
                  </div>
                  {/* Emplacement */}
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, textTransform: "uppercase" as const, letterSpacing: "0.05em", marginBottom: 4 }}>Emplacement</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <input value={newStockLoc} onChange={e => { setNewStockLoc(e.target.value); setNewStockLocId(null); }}
                        onKeyDown={e => { e.stopPropagation(); if (e.key === "Enter") lookupNewStockLoc(); }}
                        placeholder="Taper ou scanner l'emplacement (ex. H41-RKC9)..."
                        style={{ flex: 1, padding: "10px 12px", border: `1.5px solid ${newStockLocId ? "#22c55e" : C.border}`, borderRadius: 8, fontSize: 14, fontFamily: "inherit", background: newStockLocId ? "#f0fdf4" : C.white, color: C.text, outline: "none" }} />
                      <button disabled={newStockSearching || !newStockLoc} onClick={lookupNewStockLoc}
                        style={{ padding: "10px 14px", background: C.blue, color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                        {newStockSearching ? "..." : "🔍"}
                      </button>
                    </div>
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" as const }}>
                    <div style={{ flex: "1.4 1 160px" }}>
                      ...
                      <input value={newStockLot} onChange={e => setNewStockLot(e.target.value)}
                        onKeyDown={e => e.stopPropagation()}
                        placeholder="N° de lot (ex. A438033)"
                        style={{ width: "100%", padding: "10px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 14, fontFamily: "inherit", background: C.white, color: C.text, boxSizing: "border-box" as const, outline: "none" }} />
                    </div>
                    <div style={{ flex: "1 1 130px" }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, textTransform: "uppercase" as const, letterSpacing: "0.05em", marginBottom: 4 }}>DLUO <span style={{ fontWeight: 400 }}>(si nouveau lot)</span></div>
                      <input type="date" value={newStockExpiry} onChange={e => setNewStockExpiry(e.target.value)}
                        onKeyDown={e => e.stopPropagation()}
                        style={{ width: "100%", padding: "9px 12px", border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 14, fontFamily: "inherit", background: C.white, color: C.text, boxSizing: "border-box" as const, outline: "none" }} />
                    </div>
                  </div>
                  {/* Quantité + créer */}
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <button onClick={() => setNewStockQty(q => String(Math.max(0, (parseFloat(q) || 0) - 1)))}
                      style={{ width: 38, height: 38, borderRadius: 8, border: `1px solid ${C.border}`, background: C.bg, fontSize: 18, cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>−</button>
                    <input type="number" min={0} value={newStockQty} onChange={e => setNewStockQty(e.target.value)}
                      onKeyDown={e => e.stopPropagation()}
                      style={{ width: 76, padding: "8px", border: `1.5px solid ${C.blue}`, borderRadius: 8, fontSize: 17, fontWeight: 700, textAlign: "center" as const, fontFamily: "inherit", outline: "none" }} />
                    <button onClick={() => setNewStockQty(q => String((parseFloat(q) || 0) + 1))}
                      style={{ width: 38, height: 38, borderRadius: 8, border: `1px solid ${C.border}`, background: C.bg, fontSize: 18, cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>+</button>
                    <button disabled={saving || !newStockLocId || !newStockQty || parseFloat(newStockQty) <= 0}
                      onClick={addNewStockLine}
                      style={{ flex: 1, marginLeft: 4, padding: "11px 0", background: saving || !newStockLocId || parseFloat(newStockQty) <= 0 ? C.textMuted : C.blue, color: "#fff", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: saving || !newStockLocId ? "default" : "pointer", fontFamily: "inherit", opacity: !newStockLocId || parseFloat(newStockQty) <= 0 ? 0.6 : 1 }}>
                      {saving ? "Création..." : "✓ Créer la ligne"}
                    </button>
                  </div>
                  <div style={{ fontSize: 11, color: C.textMuted, marginTop: 8 }}>
                    Si le lot existe déjà sur ce produit, il est réutilisé (sa DLUO n'est pas modifiée). La nouvelle ligne apparaît dans le tableau après création.
                  </div>
                </div>
              )
            )}
...
function SettingsScreen({ onBack, session, isDark, onToggleDark }: { onBack: () => void; session: any; isDark: boolean; onToggleDark: (v: boolean) => void }) {
  ...
  // Charger les rapports quand on ouvre la section packingslip
  useEffect(() => {
    if (openType === "packingslip" && reportList.length === 0 && !loadingReports) {
      fetchReports();
    }
  }, [openType]);
...
const batchPrint = async () => {
  ...
  return (<div>
    ...
    {view === "menu" && (<div>
      <BigButton icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/></svg>} label="Palettes" sub="Scanner, remplir, rechercher, ajuster" color="#0f766e" onClick={() => setView("scan")} />
      <div style={{ height: 10 }} />
